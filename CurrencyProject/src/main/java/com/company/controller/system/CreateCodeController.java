...
public class CreateCodeController extends BaseController {
    ...
    public void add(HttpServletResponse response) throws Exception{
        ...
        List<String[]> fieldAndNameList = new ArrayList<String[]>(); //字段和备注名称
        List<String[]> commentList = new ArrayList<String[]>(); //备注名称

        List<PageData> columnList = createcodeService.columnlist(tablename);

        String priStr="";
        for (int i = 0; i < columnList.size(); i++) {
            PageData pagedate = (PageData)columnList.get(i);
            String column_name = pagedate.getString("column_name");
            String colcom = pagedate.getString("column_comment");

            String isPri = pagedate.getString("column_key");
            if(!isPri.equals("PRI")){
                fieldList.add(column_name.split(","));
                commentList.add(colcom.split(","));
            }else{
                priStr = pagedate.getString("column_name");
            }
        }
        ...
        for (int i = 0; i < columnList.size(); i++) {
            ...
            String colcom = pagedate.getString("column_comment");
            String isPri = pagedate.getString("column_key");
            String colStr = column_name+","+colcom;
            if(!isPri.equals("PRI") && !colcom.equals("")){
                fieldAndNameList.add(colStr.split(","));
            }
        ...
        root.put("priStr", priStr);
        root.put("fieldAndNameList", fieldAndNameList);
        root.put("parmaterNameLower", objectName.substring(0, 1).toLowerCase()+objectName.substring(1,objectName.length()));
